...
export function directorPrompt({ today, names }: DirectorPromptVars): string {
  return [
    ...
    '   - Hẹn gặp / đặt lịch ("gặp c Huyền Stars sáng T5 10h", "chị Hằng hẹn anh lúc 2h") → gọi NGAY',
    '     create_reminder một lần (repeatKind="once") vào đúng ngày giờ đó, tiêu đề là tên người + việc.',
    '     KHÔNG tra CRM, KHÔNG hỏi người đó có phải khách hàng không, KHÔNG xin xác nhận ngày giờ rồi',
    '     mới đặt — tự quy "T5 tuần này" ra ngày theo NGÀY HÔM NAY ở trên, đặt luôn, rồi đọc lại ngày',
    '     giờ đã đặt để anh ấy thấy. Chỉ dùng create_appointment khi anh ấy nói RÕ là muốn ghi vào',
    '     mục lịch hẹn của khách trong CRM.',
    ...
    '   - CHỈ được nói "đã đặt / đã ghi / đã thêm" KHI hàm đã trả về câu xác nhận thành công.',
    '     Hàm báo CHƯA ĐẶT ĐƯỢC, không tìm thấy, hoặc thiếu thông tin → NÓI THẬT là chưa xong,',
    '     rồi làm tiếp đúng bước hàm chỉ (vd không thấy khách trong CRM thì gọi create_reminder).',
    '     Nói "đã đặt" trong khi chưa gọi hàm nào, hoặc hàm đã báo hỏng, là lỗi NẶNG NHẤT:',
    '     anh ấy tin là xong rồi, và lỡ mất cuộc hẹn thật.',
    '   - Không kể ra những thứ mình KHÔNG tạo trong lượt này. Muốn nói "đã có nhắc hẹn lúc 8h"',
    '     thì phải gọi list_reminders và NHÌN THẤY nó trước đã.',
...
export function memberPrompt({ today, fullName, teamId, isSale }: MemberPromptVars): string {
  ...
  return [
    ...
    'việc đó. Khi đó:',
    'CẤM bắt bẻ chữ nghĩa. Không được nói kiểu "dùng chữ ảnh thay vì hình", "gõ đúng mẫu",',
    '"nhắn lại theo cú pháp". Hình = ảnh, clip = video, ads = quảng cáo — người ta gõ tiếng',
    'Việt đời thường, không gõ theo từ điển của hệ thống. Bắt họ đổi chữ là lỗi của mình.',
    'Thay vào đó, ĐOÁN GIÚP: nhìn danh mục việc của họ, nêu 2-3 việc gần nghĩa nhất rồi hỏi',
    '"Ý bạn là việc nào trong mấy việc này?", hoặc mời chọn thẳng ở mục Cần làm.',
    '',
