...
import { findByEmail, findById, publicMember } from './members.repo.js';
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const m = await findByEmail(email);
    if (!m || !m.active) throw new ApiError(401, 'Sai email hoặc mật khẩu');
    const ok = await verifyPassword(password, m.passwordHash);
    if (!ok) throw new ApiError(401, 'Sai email hoặc mật khẩu');
    res.json({
